...
public:
	...
	struct DBInfo
	{
		...
		CString DB_USER = L"root";
		CString DB_PASS = L"luken2007";
...
public:
	...
	void m_fnConnectDB();
	void m_fnCloseDB();
	void GetDBData(const char* query, std::vector<std::vector<CString>>& DBList, CListCtrl& m_List);
	void SaveDBData(std::vector<std::vector<CString>>& DBList, int i, int num,CListCtrl& m_List);
};
